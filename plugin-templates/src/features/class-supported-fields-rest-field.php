<?php
/**
 * Create WordPress Plugin Features: Supported_Fields_REST_Field class file
 *
 ...
 * supported REST API fields.
 */
final readonly class Supported_Fields_REST_Field implements Feature {
	/**
	 * Boot the feature.
	 */
	...
	public function on_rest_api_init(): void {
		register_rest_field(
			...
			[
				'schema'       => [
					'context'     => [ 'edit' ],
					'description' => __( 'Supported REST API fields for the type.', 'create-wordpress-plugin' ),
					'type'        => 'array',
					'items'       => [
						'type' => 'string',
					],
				...
				'get_callback' => function ( $response_data ): array {
					...
					if ( is_array( $response_data ) && isset( $response_data['slug'] ) && is_string( $response_data['slug'] ) ) {
						...
						$schema_properties = is_array( $data ) ? traverse( $data, 'schema.properties' ) : [];

						if ( is_array( $schema_properties ) ) {
							$out = array_keys( $schema_properties );
						}
					}
